...
public class AsyncPlayerChatListener extends Listener {
	...
	public void onChat(AsyncPlayerChatEvent e) {
		...
		if (e.getMessage().equalsIgnoreCase("maploc")) Bukkit.getConsoleSender().sendMessage("  new Location(Locations.map_world, "+(float)loc.getX()+"f, "+(int)loc.getY()+"f, "+(float)loc.getZ()+"f, "+(int)loc.getYaw()+", "+(int)loc.getPitch()+");");
