...
public class AsyncPlayerChatListener extends Listener {
	...
	public void onChat(AsyncPlayerChatEvent e) {
		...
		if (e.getMessage().equalsIgnoreCase("rloc")) Bukkit.getConsoleSender().sendMessage("  new Location(MAP_WORLD, "+(int)loc.getX()+".5f, "+(int)loc.getY()+".5f, "+(int)loc.getZ()+".5f, "+(int)loc.getYaw()+", "+(int)loc.getPitch()+");");
		else {
